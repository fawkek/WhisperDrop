...
- Release metadata defaults to version `0.2.1`, build `3`, and the current 12-character commit. Override with `APP_VERSION` and `APP_BUILD`.
...
- Download model files through the app's resumable `URLSessionDataDelegate` service. It reports exact cumulative bytes for every received network block and writes each block directly to the durable `.partial` file before reporting it. Interrupted transfers resume that same file with HTTP `Range`; never stage an entire network attempt in a disposable file because a connection loss would make visible progress roll backward. Do not estimate progress or poll the filesystem. Legacy WhisperKit/Hugging Face partial downloads are migrated once before the first new transfer, and obsolete `.partial.incoming` files from builds before 0.2.1 are removed. Keep only volume, total, and percentage visible; do not add speed or ETA unless explicitly requested.
...
- BaseRT Q4 model download size is exactly `430,114,816` bytes. Downloads use the same `ResumableFileTransfer` with HTTP `Range`, writing every received block directly to the persistent `.partial` file so an interruption preserves real progress. A CDN response that ignores a range request must preserve existing progress. Never delete a partial download merely because a duplicate tail was received: trim it to the expected length and then validate/load it.
...
- Completed transcription cues must be written atomically to `~/Library/Application Support/WhisperDrop/Recovery/CurrentSubtitleDraft.json` before proofreading starts. Restore this draft to the finished screen after a crash or relaunch, update it after successful proofreading, and clear it only when the user starts another media file or presses `Другой файл` / `Another file`.
